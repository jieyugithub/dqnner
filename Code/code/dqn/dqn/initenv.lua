...
require 'torch'
require 'nn'        ---lutm: torch neural network package
require 'nngraph'
require 'nnutils'
require 'Scale'
...
function setup(_opt)
    ...
    -- agent options
    _opt.agent_params.actions   = gameActions           --- lutm: set args for agent
    ...
    if not _opt.agent_params.state_dim then
        _opt.agent_params.state_dim = gameEnv:nObsFeature()
                                                        ...
                                                        --- lutm: _opt.agent=NeuralQLearner, _opt.agent_params有好几十个
    local agent = dqn[_opt.agent](_opt.agent_params)    --- lutm: local nql = torch.class('dqn.NeuralQLearner');
                                                        --- lutm: function nql:__init(args)
    if opt.verbose >= 1 then
...
function str_to_table(str)
    ...
    if not str or type(str) ~= 'string' then
        if type(str) == 'table' then            --- lutm: duplicate codes
            return str
        end
        return {}
    end
