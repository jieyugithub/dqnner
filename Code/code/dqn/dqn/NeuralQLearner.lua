...
local nql = torch.class('dqn.NeuralQLearner') ---lutm: Create a class. name是package.className这种形式，那么className将会加入到指定的package里面
...
function nql:__init(args)
    ...
    self.network    = args.network or self:createNetwork()      ---这里使用了network.lua里面的函数创建网络
    ...
    self.w, self.dw = self.network:getParameters()      --- lutm: w and dw are created with value network:getParameters().
    self.dw:zero()
...
function nql:perceive(reward, rawstate, terminal, testing, testing_ep)      ---lutm: This function is called twice in train_agent.lua.
...
function nql:eGreedy(state, testing_ep)
    ...
    if torch.uniform() < self.ep then
        --- torch.random(min, max) 返回属于[min, max]的一个数
        --- 因此动作的索引是从1开始的
...
function nql:createNetwork()    ---看起来像是Atari的网络
    local n_hid = 128
