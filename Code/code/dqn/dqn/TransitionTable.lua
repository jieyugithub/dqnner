...
local trans = torch.class('dqn.TransitionTable')        ---lutm: dqn is a package.
...
function trans:__init(args)
    ...
    self.histType = args.histType or "linear"           ---lutm: What is the difference of effect between the histTypes?
    self.histSpacing = args.histSpacing or 1            ---lutm: recentMemSize = histSpacing*histLen
    ...
    if self.histType == "linear" then                   ---lutm: 线性
        -- History is the last histLen frames.
        self.recentMemSize = self.histSpacing*histLen
        for i=1,histLen do
            self.histIndices[i] = i*self.histSpacing    ---lutm: histSpaceing是单位大小？
        end
    ...
    self.o = torch.LongTensor(self.maxSize):fill(0) --objects/queries
